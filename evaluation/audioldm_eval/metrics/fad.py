...
class FrechetAudioDistance:
    ...
    def __get_model(self, use_pca=False, use_activation=False):
        ...
        local_repo = Path("/mnt/petrelfs/taoye/.cache/torch/hub/torchvggish").as_posix()

        try:
            # 优先本地
            self.model = torch.hub.load(local_repo, "vggish", source="local")
        except Exception as e_local:
            # 兜底：原逻辑（若你的环境后续能联网或已有cache）
            self.model = torch.hub.load("harritaylor/torchvggish", "vggish")
        if not use_pca:
